...
T <- 400 ## dimension of the functional domain
...
for(iter in 1:nsim){
  ...
  ## fit the fmm2d model 
  ptm <- proc.time()
  fit_fmm2d <- fmm2d(formula=Y ~ X , data=data, S=S, smoother="te", knots=c(S-5, min(round(T/4, 35))),
  ...
  fit_lfosr3s <- lfosr3s(formula = Y ~ X + Longit  + (1 + Longit | ID), data = data, family = "gaussian", 
                         var = TRUE, analytic = TRUE, parallel = TRUE, silent = FALSE)
  ...
  fit_filf <-  fgee(formula = Y ~ X, Y= filf.data$Y, Cov=filf.data$X,
                    ...
                    control=list(FPCA1.pve=0.95, FPCA2.pve=0.95, FPCA1.knots= min(round(T/4, 35)), FPCA2.knots=S-6),
                    corstr = "exchangeable")
...
save(MISE_fmm2d, MISE_lfosr3s, MISE_pffr, MISE_filf,
     ...
     file = "codes/simu_results/simu2S2biT_10010100rho0.5.RData")

load("codes/simu_results/simu2S2biT_10010100rho0.5.RData")
...
fig1 <- ggplot(sub.avg, aes(x, y, fill= avg)) + 
  ...
  scale_fill_gradient2(low="darkblue", mid="grey97", midpoint = 0,
                       high="darkred", name = "Cov") + 
...
for(s in 1:10){
  cov.lfosr3s[((s-1)*100+1):(s*100),((s-1)*100+1):(s*100)] <- fit_lfosr3s$betaHat.var[1:100,1:100,2]
}
...
ret <- load_results("rho", val=c(0.5, 2, 6), iftime = TRUE, 
                    dir=c("simu2S2uniT_10010100rho0.5", 
                          "simu2S2uniT_10010100rho2",
                          "simu2S2uniT_10010100rho6") )
...
p2 <- ggplot(ret$cover, aes(x=factor(rho), y=beta, fill = method)) + 
  geom_boxplot() + 
  theme_bw() +
  scale_fill_discrete(name = "Method") +
  labs(y = expression(paste("Coverage(", hat(beta)[1], ")")), x =  expression(rho), 
...
p3 <- ggplot(ret$MIAW, aes(x=factor(rho), y=beta, fill = method)) + 
  geom_boxplot() + 
  theme_bw() +
  scale_fill_discrete(name = "Method") +
  labs(y = expression(paste("MIAW(", hat(beta)[1], ")")), x =  expression(rho), 
...
p4 <- ggplot(ret$time, aes(x=factor(rho), y=elapsed/3600, fill = method)) + 
  geom_boxplot() + 
  theme_bw() +
  scale_fill_discrete(name = "Method") +
  labs(y = "Computing Time (hours)", x = expression(rho), 
...
ggarrange(p1, p2, p4, ncol=3, nrow=1, align = "hv", 
